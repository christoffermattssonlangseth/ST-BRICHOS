"""Generate notebooks/01/08-canonical-orientation.ipynb."""
from pathlib import Path

import nbformat as nbf

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "notebooks" / "01" / "08-canonical-orientation.ipynb"
...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "\n"
    "Goal: rotate each sample so all coronal sections face the same way "
    "in figure panels. Iterate by eye on this notebook, then write an "
    "oriented combined h5ad that downstream notebooks read.\n"
    "\n"
    "Workflow\n"
...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "\n"
    "Crosshair marks image centre — handy for judging tilt against the "
    "midline. Spot dots coloured by treatment."
))
...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "\n"
    "Initial values loaded from `data/rotations.yaml`. Edit, re-run the "
    "preview cell below, repeat. When happy, run the save cell."
))
...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "\n"
    "Works on a deep copy so the original `adata` stays clean. Re-run after "
